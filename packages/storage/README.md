...
`StorageAuditHistory` writes immutable, queryable `AuditEvent` records and reports duplicate IDs as a structured `AuditHistoryError`. `AuditedOperationRunner` requires a durable pending event before invoking a manager or other operation, then requires a terminal event and explicitly reports whether an operation already completed when that final write fails. Logical operation IDs remain stable across recovery while every attempt has a unique ID. Payloads are caller-normalized JSON: applications must redact credentials, OAuth tokens, provider payload secrets, and model-sensitive data before appending them. Production deployments should apply retention, backup, capacity, and access-control policies described in `docs/OPERATIONS.md`.
